...
function getAuthErrorMessage(error: { message?: string; status?: number; name?: string }): string {
  ...
  if (error.status === 400) return "Invalid authentication request.";
  if (error.status === 401) return "Authentication failed. Please check your details.";
...
function getNetworkErrorMessage(error: unknown): string {
  if (error instanceof TypeError) {
    ...
    if (message.includes("failed to fetch") || message.includes("network")) return "Unable to connect to Supabase. Please check your Vercel Supabase environment variables and redeploy the application.";
  }
  if (error instanceof Error) return error.message || "Authentication request failed.";
