...
export default function PaymentCallback() {
  ...
  async function verifyPayment() {
    try {
      ...
      // IMPORTANT: Pass an object with both transactionId and txRef as required by verifyWalletFunding
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white flex items-center justify-center px-4 py-8">
      <div className="w-full max-w-md">
        ...
        <div className="rounded-2xl border border-slate-800 bg-slate-900/90 p-6 shadow-2xl sm:p-8">
            ...
            <div className="text-center">
              ...
              {reference && (
                <div className="mt-6 rounded-xl bg-slate-950 p-4 text-left">
                  ...
                  <p className="mt-1 break-all text-sm text-slate-300">{reference}</p>
                </div>
              )}
              <p className="mt-5 text-xs text-slate-600">Please do not close this page.</p>
            </div>
          )}
          ...
          {status === "success" && (
            <div className="text-center">
              ...
              <button type="button" onClick={returnToWallet} className="mt-6 flex w-full items-center justify-center gap-2 rounded-xl bg-emerald-500 px-4 py-3 text-sm font-semibold text-slate-950 transition-colors hover:bg-emerald-600">
          ...
          {status === "failed" && (
            <div className="text-center">
              ...
              <button type="button" onClick={returnToWallet} className="mt-6 flex w-full items-center justify-center gap-2 rounded-xl border border-slate-700 bg-slate-800 px-4 py-3 text-sm font-semibold transition-colors hover:bg-slate-700">
                <ArrowLeft className="h-4 w-4" /> Return to Wallet
              </button>
            </div>
          )}
        ...
        <p className="mt-6 text-center text-xs leading-5 text-slate-600">Payment verification is handled securely by CheapDataHub.</p>
      </div>
    </div>
  );
}
